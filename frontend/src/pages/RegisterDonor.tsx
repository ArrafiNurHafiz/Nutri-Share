...
export function RegisterDonor() {
  ...
  const validateField = (field: string, value: string) => {
    const rules: Record<string, () => { valid: boolean; message?: string }> = {
      business_name: () => validateRequired(value, "Business Name"),
  ...
  const validateAll = (): boolean => {
    const fields = [
      {
        key: "business_name",
        fn: () => validateRequired(form.business_name, "Business Name"),
      },
      ...
      >
        ...
        <div className="absolute bottom-8 left-8 right-8 p-6 md:p-8 backdrop-blur-md bg-white/15 rounded-2xl border border-white/20 z-10">
          ...
          <p className="text-white/85 leading-relaxed max-w-lg text-sm">
            Join food-conscious businesses turning surplus into smiles. Every
            donation counts.
          </p>
          <div className="mt-4">
            <span className="inline-block px-3 py-1 bg-primary-orange/30 text-white text-xs font-bold rounded-full">
              Become a Food Hero
            </span>
          </div>
        </div>
        ...
        >
          ...
          <div className="mb-6">
            ...
            <p className="text-gray-500">
              List your business and start donating surplus food.
            </p>
          </div>
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="sm:col-span-2">
                ...
                <div className="relative">
                  ...
                  />
                  <input
                    placeholder="e.g. Aston Hotel"
                    value={form.business_name}
                    onChange={(e) => update("business_name", e.target.value)}
                    onBlur={() =>
                      validateField("business_name", form.business_name)
                    }
                    className={`w-full border rounded-xl pl-10 pr-4 py-3 bg-[#f8fafc] border-gray-200 focus:ring-2 focus:ring-primary-orange/20 focus:border-primary-orange outline-none transition-all ${getErrorClass(errors.business_name)}`}
                    required
                  />
                </div>
            ...
            <div className="bg-[#f8fafc] border border-gray-200 rounded-2xl p-5">
              <h3 className="text-sm font-bold text-brand-dark mb-2 flex items-center gap-2">
                <MapPin size={16} className="text-brand-medium" /> Pickup
                Location
              </h3>
