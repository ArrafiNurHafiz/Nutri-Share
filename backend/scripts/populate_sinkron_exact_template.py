...
def populate_sinkron_template():
    ...
    # 1. Update Title of Paper in the Checklist Form Header
    ...
    # 2. Pre-fill Checklist Table 0 (Author Claim & Comment column) with "OK"
    if len(doc.tables) > 0:
        t0 = doc.tables[0]
    ...
    # 3. Clean up the placeholder body paragraphs after the Reviewer tables
    # Find the paragraph where manuscript body starts
    # We want to remove all empty blank paragraphs between Reviewer 3 and Title
    cutoff_idx = 0
    for idx, p in enumerate(doc.paragraphs):
        if "Reviewer # 3" in p.text:
            cutoff_idx = idx + 3
            break

    # Remove trailing paragraphs after Reviewer 3
    for pe in [p._element for p in doc.paragraphs[cutoff_idx:]]:
        pe.getparent().remove(pe)

    # Add a page break to start manuscript on a fresh page
    doc.add_page_break()

    # --- MANUSCRIPT BODY ---

    # TITLE (Sinkron requirement: 24pt, Regular, max 12 words, Title Case)
    p_title = doc.add_paragraph()
    p_title.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p_title.paragraph_format.space_before = Pt(12)
    p_title.paragraph_format.space_after = Pt(12)
    r_t = p_title.add_run("Implementation of Hybrid Shannon Entropy-TOPSIS for Multi-Criteria Surplus Food Distribution")
    r_t.font.name = "Times New Roman"
    r_t.font.size = Pt(20)
    r_t.bold = True
    r_t.font.color.rgb = RGBColor(0x04, 0x78, 0x57)

    # AUTHOR
    p_author = doc.add_paragraph()
    p_author.alignment = WD_ALIGN_PARAGRAPH.CENTER
    p_author.paragraph_format.space_after = Pt(2)
    r_a = p_author.add_run("Arrafi Nur Hafiz1)*")
    ...
    r_af = p_aff.add_run("1)Department of Information Systems, Faculty of Information Technology, Universitas Teknologi Digital Indonesia, Yogyakarta, Indonesia")
    ...
    r_em = p_em.add_run("1)*arrafinur3@gmail.com")
    ...
    # ABSTRACT (214 words, 1 paragraph, no abbreviations, no trademarks, no citations)
    abstract_text = (
    ...
    r_absh.bold = True
    r_absb = p_abs.add_run(abstract_text)
    r_absb.font.name = "Times New Roman"
    r_absb.font.size = Pt(10)

    # KEYWORDS (Minimal 5 keywords, semicolon separated)
    ...
    r_kwh.bold = True
    r_kwb = p_kw.add_run("Decision Support System; Surplus Food; Hybrid TOPSIS; Shannon Entropy; Haversine Distance; Humanitarian Logistics; Food Waste Reduction")
    ...
    # -------------------------------------------------------------
    h_intro = doc.add_paragraph(style='Heading 1')
    h_intro.paragraph_format.space_before = Pt(14)
    h_intro.paragraph_format.space_after = Pt(4)
    r = h_intro.add_run("INTRODUCTION")
    ...
    p_in1.add_run(
        "Food loss and waste alongside nutritional insecurity represent one of the most pressing socio-economic paradoxes in developing nations (Badan Pangan Nasional, 2023). "
    ...
    p_in2.add_run(
        "Conventional surplus food redistribution predominantly relies on ad-hoc phone calls, social media messaging, or First-Come, First-Served (FCFS) manual claiming (Sinnott et al., 2022). "
        ...
        "Second, perishable cooked foods possess brief consumption windows (often less than 4 to 6 hours); ignoring geographical proximity and transport logistics results in severe spoilage before delivery (Ozkir & Demirel, 2022). "
        ...
        "Hence, an automated and intelligent multi-criteria Decision Support System (DSS) is vital to optimize matching speed, logistical feasibility, and humanitarian fairness (Wulandari & Hartono, 2024)."
    )
    ...
    p_in3.add_run(
        ...
        "coupled with a Hybrid Shannon Entropy weighting mechanism and the Haversine distance formula (Kurniawan & Sunardi, 2022). The system resolves three central research questions: "
    ...
    # -------------------------------------------------------------
    h_lr = doc.add_paragraph(style='Heading 1')
    h_lr.paragraph_format.space_before = Pt(14)
    h_lr.paragraph_format.space_after = Pt(4)
    r = h_lr.add_run("LITERATURE REVIEW")
    r.font.name = "Times New Roman"
    r.font.size = Pt(12)
    r.bold = True
    ...
    p_lr1.add_run(
        ...
        "and distance from the Negative Ideal Solution (NIS). In humanitarian relief and food logistics, TOPSIS has demonstrated exceptional computational efficiency and conceptual clarity (Liu & Zhang, 2021; Susanto & Hermawan, 2023). "
    ...
    p_lr3.add_run(
        ...
        "the proposed architecture guarantees that baseline organizational policies remain active even during data homogeneity, while preserving data-driven flexibility (Utomo & Wahyuni, 2023). "
    ...
    # 3. METHOD (Heading 1)
    # -------------------------------------------------------------
    h_m = doc.add_paragraph(style='Heading 1')
    h_m.paragraph_format.space_before = Pt(14)
    h_m.paragraph_format.space_after = Pt(4)
    r = h_m.add_run("METHOD")
    r.font.name = "Times New Roman"
    r.font.size = Pt(12)
    r.bold = True
    ...
    p_eq4.paragraph_format.space_after = Pt(2)
    ...
    p_eq6.paragraph_format.space_after = Pt(4)
    ...
    p_eq7.paragraph_format.space_after = Pt(4)
    ...
    p_eq8.paragraph_format.space_after = Pt(4)
    ...
    p_eq10.paragraph_format.space_after = Pt(4)
    ...
    p_eq13.paragraph_format.space_after = Pt(4)
    ...
    # 4. RESULT (Heading 1)
    # -------------------------------------------------------------
    h_res = doc.add_paragraph(style='Heading 1')
    h_res.paragraph_format.space_before = Pt(14)
    h_res.paragraph_format.space_after = Pt(4)
    r = h_res.add_run("RESULT")
    r.font.name = "Times New Roman"
    r.font.size = Pt(12)
    r.bold = True
    ...
    # -------------------------------------------------------------
    # 5. DISCUSSIONS (Heading 1)
    # -------------------------------------------------------------
    h_disc = doc.add_paragraph(style='Heading 1')
    h_disc.paragraph_format.space_before = Pt(14)
    h_disc.paragraph_format.space_after = Pt(4)
    r = h_disc.add_run("DISCUSSIONS")
    r.font.name = "Times New Roman"
    r.font.size = Pt(12)
    r.bold = True
    ...
    # -------------------------------------------------------------
    h_concl = doc.add_paragraph(style='Heading 1')
    h_concl.paragraph_format.space_before = Pt(14)
    h_concl.paragraph_format.space_after = Pt(4)
    r = h_concl.add_run("CONCLUSION")
    r.font.name = "Times New Roman"
    r.font.size = Pt(12)
    r.bold = True
    ...
    # -------------------------------------------------------------
    # ACKNOWLEDGMENT(optional) (Heading 1)
    # -------------------------------------------------------------
    h_ack = doc.add_paragraph(style='Heading 1')
    h_ack.paragraph_format.space_before = Pt(14)
    h_ack.paragraph_format.space_after = Pt(4)
    r = h_ack.add_run("ACKNOWLEDGMENT")
    r.font.name = "Times New Roman"
    r.font.size = Pt(12)
    r.bold = True
    ...
    p_ack.add_run(
        "The author expresses sincere gratitude to the Department of Information Systems and the Faculty of Information Technology at Universitas Teknologi Digital Indonesia (UTDI) "
        "for the academic resources and research environment provided during the development of this project. Appreciation is also extended to local social welfare institutions in Yogyakarta for domain insights."
    )

    # -------------------------------------------------------------
    # REFERENCES (APA CITATION FORMATTING STYLE) (Heading 1)
    # -------------------------------------------------------------
    h_ref = doc.add_paragraph(style='Heading 1')
    h_ref.paragraph_format.space_before = Pt(14)
    h_ref.paragraph_format.space_after = Pt(4)
    r = h_ref.add_run("REFERENCES (APA CITATION FORMATTING STYLE)")
    r.font.name = "Times New Roman"
    r.font.size = Pt(12)
    r.bold = True
    ...
    print(f"Deeply polished Sinkron template successfully saved at: {output_path}")

if __name__ == "__main__":
    populate_sinkron_template()
